...
@section('title', 'Editar Promoción - FarmaBien')
...
<div class="space-y-4" x-data="{
    ...
    valor: {{ old('valor', $promocion->valor) }},
    minUnidades: {{ old('min_unidades', $promocion->min_unidades) }},
    samplePrice: 20.00,
    updateTipo(newTipo) {
        this.tipo = newTipo;
        if (newTipo === '2x1') {
    ...
    <nav class="flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400">
        ...
        <svg class="w-3 h-3 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <span class="text-slate-800 dark:text-slate-200 font-semibold">Editar Promoción #{{ $promocion->id }}</span>
    </nav>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-1 border-b border-slate-300/80 dark:border-slate-800">
        <div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                <span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                    {{ $promocion->nombre }}
                </span>
            </h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Modifica las reglas de descuento, fechas de vigencia o alcance del beneficio.
            </p>
        </div>
    ...
    <!-- Form & Live Simulator -->
    <form action="{{ route('promociones.update', $promocion) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="nombre" value="{{ old('nombre', $promocion->nombre) }}" required maxlength="150"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="descripcion" rows="2" maxlength="1000"
                                      class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white text-xs focus:ring-emerald-500 focus:border-emerald-500">{{ old('descripcion', $promocion->descripcion) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <label class="flex flex-col items-center p-3 rounded-xl border cursor-pointer transition text-center"
                            ...
                            <input type="radio" name="tipo" value="porcentaje" class="sr-only" @click="updateTipo('porcentaje')" {{ old('tipo', $promocion->tipo) === 'porcentaje' ? 'checked' : '' }}>
                        ...
                        <label class="flex flex-col items-center p-3 rounded-xl border cursor-pointer transition text-center"
                            ...
                            <input type="radio" name="tipo" value="monto_fijo" class="sr-only" @click="updateTipo('monto_fijo')" {{ old('tipo', $promocion->tipo) === 'monto_fijo' ? 'checked' : '' }}>
                        ...
                        <label class="flex flex-col items-center p-3 rounded-xl border cursor-pointer transition text-center"
                            ...
                            <input type="radio" name="tipo" value="2x1" class="sr-only" @click="updateTipo('2x1')" {{ old('tipo', $promocion->tipo) === '2x1' ? 'checked' : '' }}>
                        ...
                        <label class="flex flex-col items-center p-3 rounded-xl border cursor-pointer transition text-center"
                            ...
                            <input type="radio" name="tipo" value="3x2" class="sr-only" @click="updateTipo('3x2')" {{ old('tipo', $promocion->tipo) === '3x2' ? 'checked' : '' }}>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                        ...
                        <div>
                            ...
                            <input type="number" min="1" name="min_unidades" x-model="minUnidades" required
                                   class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white text-sm focus:ring-emerald-500 focus:border-emerald-500">
                            <p class="text-[10px] text-slate-400 mt-0.5">Cantidad mínima para que aplique.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" min="1" name="stock_limite" value="{{ old('stock_limite', $promocion->stock_limite) }}"
                                   placeholder="Ilimitado"
                                   class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white text-sm focus:ring-emerald-500 focus:border-emerald-500">
                            <p class="text-[10px] text-slate-400 mt-0.5">Consumidas: {{ $promocion->stock_consumido }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs p-5 space-y-4">
                    ...
                    <div x-show="alcance === 'producto'" x-cloak class="pt-2">
                        ...
                        <select name="producto_id" class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white text-xs focus:ring-emerald-500 focus:border-emerald-500">
                            ...
                            <option value="{{ $prod->id }}" {{ old('producto_id', $promocion->producto_id) == $prod->id ? 'selected' : '' }}>
                    ...
                    <div x-show="alcance === 'laboratorio'" x-cloak class="pt-2">
                        ...
                        <select name="laboratorio_id" class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white text-xs focus:ring-emerald-500 focus:border-emerald-500">
                            ...
                            <option value="{{ $lab->id }}" {{ old('laboratorio_id', $promocion->laboratorio_id) == $lab->id ? 'selected' : '' }}>
                                {{ $lab->nombre }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="datetime-local" name="fecha_inicio" required
                                   value="{{ old('fecha_inicio', $promocion->fecha_inicio ? $promocion->fecha_inicio->format('Y-m-d\TH:i') : '') }}"
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="fecha_fin" required
                                   value="{{ old('fecha_fin', $promocion->fecha_fin ? $promocion->fecha_fin->format('Y-m-d\TH:i') : '') }}"
                                   class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white text-xs focus:ring-emerald-500 focus:border-emerald-500">
                        </div>

                        <div class="sm:col-span-2 pt-2">
                            <label class="inline-flex items-center space-x-2.5 cursor-pointer">
                                <input type="checkbox" name="activo" value="1" {{ old('activo', $promocion->activo) ? 'checked' : '' }}
                                       class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
                                <span class="text-xs font-semibold text-slate-800 dark:text-slate-200">Promoción activa en el sistema</span>
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <!-- Columna Derecha: Simulador de Descuento -->
            <div class="space-y-4">
