...
<div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-1 border-b border-slate-300/80 dark:border-slate-800 print:hidden">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <a href="{{ route('reportes.recetas', array_merge(request()->query(), ['export' => 'pdf'])) }}" target="_blank"
            ...
            <a href="{{ route('reportes.recetas', array_merge(request()->query(), ['export' => 'excel'])) }}"
    ...
    <div class="hidden print:block border-b-2 border-sky-600 pb-3 mb-4">
        <div class="flex items-start justify-between">
            ...
            <div class="text-right">
                <div class="inline-block border border-sky-600 bg-sky-50 px-3 py-1.5 rounded text-center">
                    <p class="text-xs font-bold text-sky-800">REPORTE DE RECETAS MÉDICAS</p>
                    <p class="text-[9px] text-sky-700 mt-0.5">Emisión: {{ now()->format('d/m/Y H:i') }}</p>
                    <p class="text-[9px] text-sky-700">Por: {{ Auth::user()->name ?? 'Sistema' }}</p>
                </div>
            </div>
        </div>
        <div class="mt-2 text-xs bg-slate-100 p-2 rounded flex items-center justify-between">
            ...
            <span><strong>Filtro Estado:</strong> {{ ucfirst($estadoFiltro ?: 'Todos') }}</span>
            <span><strong>Total Recetas:</strong> {{ number_format($totalRecetas) }}</span>
        </div>
    </div>
