...
@section('title', 'Nuevo Proveedor - FarmaBien')
...
@keydown.window="if ($event.key === 'Escape' && formLayout === 'compact') { window.location.href = '{{ route('proveedores.index') }}'; }"
...
class="space-y-4 transition-all duration-200">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
            <h1 class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">Registrar Nuevo Proveedor / Droguería</h1>
            <p class="text-xs text-slate-600 dark:text-slate-400">
                Ingresa los datos fiscales, comerciales y de contacto del distribuidor.
            </p>
        </div>
    ...
    <!-- Form -->
    <form x-ref="proveedorForm" method="POST" action="{{ route('proveedores.store') }}">
        @csrf
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-md p-4 space-y-4 animate-fadeIn">
                ...
                <div class="flex items-center justify-between pb-3 border-b border-slate-300 dark:border-slate-800 bg-slate-100/80 dark:bg-slate-800/60 -mx-4 -mt-4 p-3 rounded-t-2xl">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-xs font-extrabold text-slate-800 dark:text-slate-200 uppercase tracking-wide">FICHA RÁPIDA DE PROVEEDOR / DROGUERÍA</span>
