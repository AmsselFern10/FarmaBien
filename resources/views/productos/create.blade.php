...
<div class="bg-white overflow-hidden shadow-sm rounded-lg">
    <form action="{{ route('productos.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
        @csrf
...
<script>
    ...
    // Calcular precio de venta sugerido (30% de margen)
    document.getElementById('precio_compra').addEventListener('change', function() {
        ...
        if (precioCompra > 0 && !precioVenta.value) {
            const sugerido = (precioCompra * 1.30).toFixed(2);
            precioVenta.value = sugerido;
        }
    });
</script>
@endpush
