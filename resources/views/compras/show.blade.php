...
{{-- Usamos la relación cargada 'reemplazadaPor' --}}
@if($compra->reemplazadaPor)
    ...
    <div class="flex">
        ...
        <div>
            <p class="text-sm font-semibold text-purple-800 dark:text-purple-300">
                ...
                <a href="{{ route('compras.show', $compra->reemplazadaPor->id) }}" class="underline hover:text-purple-900 dark:hover:text-purple-200">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <!-- Información General (más legible) -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-white dark:from-gray-800 dark:to-gray-800/50">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h3 class="text-xl font-semibold text-slate-900 dark:text-white">Información de la Compra</h3>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Fecha de Compra</p>
                        <p class="text-lg font-bold text-slate-900 dark:text-white">{{ $compra->fecha->format('d/m/Y') }}</p>
                    </div>

                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Estado</p>
                    ...
                    @php
                        $descuentoCompra = (float)($compra->descuento ?? 0);
                        $descuentoCompra = max(0, min(100, $descuentoCompra));
                    @endphp
                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Descuento (Global)</p>
                        <p class="text-lg font-bold text-slate-900 dark:text-white">{{ number_format($descuentoCompra, 2) }}%</p>
                    </div>

                    <div class="md:col-span-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-2">Proveedor</p>
                        <p class="text-lg font-bold text-slate-900 dark:text-white">{{ $compra->proveedor->nombre }}</p>
                        <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">RUC: {{ $compra->proveedor->ruc }}</p>
                    </div>

                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Registrado por</p>
                        <p class="text-base font-semibold text-slate-900 dark:text-white">{{ $compra->usuario->name }}</p>
                    </div>

                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Fecha de Registro</p>
                        <p class="text-base font-semibold text-slate-900 dark:text-white">{{ $compra->created_at->format('d/m/Y H:i') }}</p>
                    </div>

                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Total (Guardado)</p>
                        <p class="text-lg font-bold text-slate-900 dark:text-white">S/ {{ number_format((float)$compra->total, 2) }}</p>
                    </div>

                    <div class="md:col-span-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-2">Observaciones</p>
                        <p class="text-base text-slate-900 dark:text-white whitespace-pre-line">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-right text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Precio</th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Desc.</th>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                            <td class="px-4 py-3">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <div class="text-xs text-slate-500 dark:text-slate-400">{{ optional($detalle->producto->categoria)->nombre ?? '—' }}</div>
                                        @php
                                            $presNombre = null;
                            ...
                            <td class="px-4 py-3 text-right">
                                @php
                                    ...
                                    $precioPres = round($precioUnit * $unidPres, 2);
                                @endphp
                                <div class="text-sm font-semibold text-slate-900 dark:text-white">S/ {{ number_format($precioPres, 2) }}</div>
                                <div class="text-xs text-slate-500 dark:text-slate-400">Unit: S/ {{ number_format($precioUnit, 2) }}</div>
                            </td>
                            <td class="px-4 py-3 text-right">
                                @php
                                    $descItem = (float)($detalle->descuento ?? 0);
                                    $descItem = max(0, min(100, $descItem));

                                    $cantPres = $detalle->cantidad_presentaciones ?? null;
                                    $totalUnid = (int)($detalle->cantidad ?? 0);
                                    $subtotalBruto = 0.0;
                                    if ($cantPres !== null) {
                                        $subtotalBruto = round($precioPres * (int)$cantPres, 2);
                                    } else {
                                        $subtotalBruto = round($precioUnit * $totalUnid, 2);
                                    }
                                    $montoDescItem = round($subtotalBruto * ($descItem / 100), 2);
                                    $subtotalNetoCalc = round($subtotalBruto - $montoDescItem, 2);
                                    $subtotalGuardado = (float)($detalle->subtotal ?? 0);
                                    $mostrarSubtotal = $subtotalGuardado > 0 ? $subtotalGuardado : $subtotalNetoCalc;
                                @endphp

                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-gray-700 text-slate-800 dark:text-slate-200 font-semibold text-sm">
                                    {{ number_format($descItem, 2) }}%
                                </span>
                                @if($montoDescItem > 0)
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mt-1">- S/ {{ number_format($montoDescItem, 2) }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-right">
                                <span class="inline-flex items-center px-3 py-1 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-bold text-sm">
                                    S/ {{ number_format($mostrarSubtotal, 2) }}
                                </span>
                                @if($descItem > 0)
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                        Bruto: S/ {{ number_format($subtotalBruto, 2) }}
                                    </div>
                                    @if(abs($subtotalGuardado - $subtotalNetoCalc) > 0.01 && $subtotalGuardado > 0)
                                        <div class="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                                            Neto calc.: S/ {{ number_format($subtotalNetoCalc, 2) }}
                                        </div>
                                    @endif
                                @endif
                            </td>
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-2 py-3 border-y border-white/10">
                    ...
                    <div class="flex justify-between items-center text-blue-100">
                        <span class="text-xs">Desc. por Items (Tabla):</span>
