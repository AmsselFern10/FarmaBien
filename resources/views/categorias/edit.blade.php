    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center space-x-2 text-xs text-slate-500 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-800 dark:text-slate-300 font-medium">Modificar</span>
            </div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Editar Categoría: {{ $categoria->nombre }}</h1>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="descripcion" 
                              ...
                              class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 focus-visible:ring-2 focus-visible:ring-emerald-500 transition @error('descripcion') border-rose-500 @enderror">{{ old('descripcion', $categoria->descripcion) }}</textarea>
                    @error('descripcion')
                        <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
