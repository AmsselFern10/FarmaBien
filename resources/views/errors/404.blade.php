...
<head>
    ...
    <title>404 - Registro No Encontrado | {{ config('app.name', 'FarmaBien') }}</title>
...
<body class="font-sans antialiased bg-slate-950 text-slate-100 min-h-screen flex items-center justify-center p-4">
    <div class="max-w-md w-full text-center bg-slate-900/90 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6">
        <div class="w-16 h-16 mx-auto bg-slate-800 border border-slate-700 rounded-2xl flex items-center justify-center text-slate-400 text-3xl font-black">
            🔍
        </div>
        <div class="space-y-2">
            <h1 class="text-3xl font-black text-white tracking-tight">404</h1>
            <h2 class="text-base font-bold text-slate-200">Recurso o Página No Encontrada</h2>
            <p class="text-xs text-slate-400 leading-relaxed">
                El medicamento, venta, lote o página que estás buscando no existe en el sistema o ha sido reubicado.
            </p>
        </div>
        ...
        <div class="border-t border-slate-800/80 pt-4 text-[10px] text-slate-400">
            Seguridad &bull; {{ config('app.name', 'FarmaBien') }}
        </div>
    </div>
</body>
</html>
