...
<div x-data="{
    ...
    formData: (window.farmaGetDraft ? window.farmaGetDraft('{{ request()->getPathInfo() }}', {
        nombre: @js(old('nombre', $cliente->nombre)),
        documento: @js(old('documento', $cliente->documento)),
        telefono: @js(old('telefono', $cliente->telefono)),
...
class="space-y-4 transition-all duration-200">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-1 border-b border-slate-300/80 dark:border-slate-800">
        <nav class="flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400">
            ...
            <svg class="w-3 h-3 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-slate-800 dark:text-slate-200 font-semibold truncate max-w-[200px]">Editar: {{ $cliente->nombre }}</span>
        </nav>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h1 class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <span>Editar Cliente: {{ $cliente->nombre }}</span>
                <span class="px-2 py-0.5 text-xs font-bold rounded-full {{ $cliente->activo ? 'bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300' : 'bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400' }}">
                    {{ $cliente->activo ? 'Activo' : 'Inactivo' }}
                </span>
            </h1>
            <p class="text-xs text-slate-600 dark:text-slate-400">
                Actualiza los datos personales, documento y medios de contacto del paciente.
            </p>
        </div>
        <div class="flex items-center space-x-2 shrink-0">
            <a href="{{ route('clientes.show', $cliente) }}" 
               class="px-3.5 py-1.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl transition shadow-2xs">
                Ver Ficha
            </a>
    ...
    <!-- Form -->
    <form x-ref="clienteEditForm" method="POST" action="{{ route('clientes.update', $cliente) }}">
        @csrf
        @method('PUT')
        ...
        <template x-if="formLayout === 'compact'">
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-md p-4 space-y-4 animate-fadeIn">
                ...
                <div class="pt-2 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500">
                    <span class="text-[11px]">Modificando cliente #{{ $cliente->id }}</span>
                    <button type="submit" 
