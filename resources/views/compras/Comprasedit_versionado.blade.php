...
@section('page-actions')
    ...
    <div class="flex gap-3">
        <button type="button" id="btnRevertirCompra"
                class="inline-flex items-center px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg font-semibold shadow-sm transition-colors duration-200">
            Revertir cambios
        </button>
    ...
    <form id="formCompra" method="POST" action="{{ route('compras.update', $compra) }}" class="space-y-4 -mt-2">
        ...
        @php
            $detallesIniciales = $compra->detalles->map(function($d) {
                $p = $d->producto;
                $codigo = $p->codigo_barras ?? ($p->codigo_barra ?? '');
                $img = ($p && $p->imagen) ? asset('storage/' . $p->imagen) : null;

                $loteNumero = $d->lote?->numero_lote ?? ($d->lote?->numero ?? ($d->lote?->lote ?? null));
                $loteVence = $d->lote?->fecha_vencimiento ?? ($d->lote?->vencimiento ?? null);
...
const COMPRA_ID = @json($compra->id);
const COMPRA_DRAFT_KEY = `compras_edit_${COMPRA_ID}_draft_v1`;
const COMPRA_VISTA_KEY = `compras_edit_${COMPRA_ID}_vista`;
...
function saveCompraDraft() {
    try {
        ...
        // Captura estado actual desde DOM
        ...
        const motivoInput = document.querySelector('textarea[name="motivo_anulacion"]');
        ...
        const payload = {
            ...
            indicesActivos,
            estadoDetalles,
            proveedor_id: proveedorSelect ? proveedorSelect.value : null,
            fecha: fechaInput ? fechaInput.value : null,
            descuento: descuentoInput ? descuentoInput.value : null,
            observaciones: obsInput ? obsInput.value : null,
            motivo_anulacion: motivoInput ? motivoInput.value : null,
        };
        ...
        // silencioso
        console.warn('No se pudo guardar el borrador de compra (edit):', e);
    }
}
...
function restoreCompraDraft() {
    ...
    if (!raw) {
        // Si no hay borrador, al menos restaura vista preferida
        const vista = localStorage.getItem(COMPRA_VISTA_KEY);
        if (vista && (vista === 'tabla' || vista === 'formulario')) {
            vistaActual = vista;
            // cambia visualmente sin capturar estado
    ...
    try {
        ...
        if (proveedorSelect && data.proveedor_id) proveedorSelect.value = data.proveedor_id;
        if (fechaInput && data.fecha) fechaInput.value = data.fecha;
        if (descuentoInput && data.descuento !== null && data.descuento !== undefined) descuentoInput.value = data.descuento;
        if (obsInput && data.observaciones !== null && data.observaciones !== undefined) obsInput.value = data.observaciones;
        if (motivoInput && data.motivo_anulacion !== null && data.motivo_anulacion !== undefined) motivoInput.value = data.motivo_anulacion;

        // Estado de detalles
        indicesActivos = Array.isArray(data.indicesActivos) ? data.indicesActivos : [];
        contadorProductos = (typeof data.contadorProductos === 'number') ? data.contadorProductos : 0;

        // limpiar objeto sin perder referencia
        Object.keys(estadoDetalles).forEach(k => delete estadoDetalles[k]);
        if (data.estadoDetalles && typeof data.estadoDetalles === 'object') {
            Object.entries(data.estadoDetalles).forEach(([k, v]) => {
                estadoDetalles[k] = v;
            });
        }

        // Vista
        const vista = data.vistaActual || localStorage.getItem(COMPRA_VISTA_KEY) || 'tabla';
        vistaActual = (vista === 'formulario') ? 'formulario' : 'tabla';
        ...
        restaurandoDraft = false;
        console.warn('No se pudo restaurar el borrador de compra (edit):', e);
    }
}

function revertirCambiosCompra() {
    clearCompraDraft();
    localStorage.removeItem(COMPRA_VISTA_KEY);
...
function sincronizarHiddenDesdeVisibles(index) {
    ...
    // Precio: input es precio por presentación; en hidden guardamos precio unitario base
    if (precio) {
        const u = parseFloat(unidades || '1') || 1;
        const pPres = parseFloat(precio.value || '0') || 0;
        const pUnit = (u > 0) ? (pPres / u) : 0;
        setHidden('precio_unitario', String(pUnit.toFixed(4)));
    }
...
function cambiarVista(vista) {
    ...
    vistaActual = vista;
    try { localStorage.setItem(COMPRA_VISTA_KEY, vistaActual); } catch (e) {}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Restaurar borrador (localStorage) si existe
    restoreCompraDraft();

    document.getElementById('btnCrearProveedor')?.addEventListener('click', abrirModalProveedor);

    document.getElementById('btnRevertirCompra')?.addEventListener('click', () => revertirCambiosCompra());

    const formCompra = document.getElementById('formCompra');
    if (formCompra) {
        formCompra.addEventListener('input', () => saveCompraDraftDebounced());
        formCompra.addEventListener('change', () => saveCompraDraftDebounced());
    }
