...
function ajusteForm() {
    return {
        ...
        sumar(n) {
            const v = parseInt(this.stockNuevo) || 0;
            this.stockNuevo = Math.max(0, v + n);
        },
        init() {
...
<div x-data="ajusteForm()" x-init="init()" class="space-y-5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5 space-y-4">
            ...
            <div x-show="lote" class="space-y-3">
    ...
    {{-- Nota informativa --}}
    <div class="px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-xs text-slate-500 dark:text-slate-400 flex items-start gap-2.5">
        <svg class="w-4 h-4 shrink-0 mt-0.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        <span>El ajuste actualiza el stock con bloqueo de concurrencia. Genera un registro <strong class="text-slate-700 dark:text-slate-300">permanente e inamovible</strong> en el Kardex. Para corregir un error, registra otro ajuste.</span>
    </div>
</div>
@endsection
