    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Gestiona las presentaciones comerciales (caja, blíster, frasco, ampolla, etc.) y venta fraccionada.
            </p>
        </div>
        @can('crear productos')
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-xs">
        <form method="GET" action="{{ route('presentaciones.index') }}" class="flex flex-col md:flex-row items-center gap-3">
            ...
            <div class="flex items-center space-x-2 w-full md:w-auto shrink-0">
                ...
                <a href="{{ route('presentaciones.index') }}" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-semibold rounded-xl transition">
                    Limpiar
                </a>
                @endif
            </div>
            ...
            <table class="w-full text-left text-xs">
                <thead class="bg-slate-50 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 font-semibold border-b border-slate-200 dark:border-slate-800 uppercase tracking-wider text-[11px]">
                    <tr>
                        ...
                        <th class="px-5 py-3.5 text-center">Código de Barras</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300">
                    ...
                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition">
                        ...
                        <td class="px-5 py-3.5">
                            @if($pres->producto)
                            <a href="{{ route('productos.show', $pres->producto) }}" class="font-medium text-slate-900 dark:text-white hover:text-emerald-600 dark:hover:text-emerald-400 transition">
                                {{ $pres->producto->nombre }}
                            </a>
                            <span class="block text-[11px] text-slate-400">{{ $pres->producto->laboratorio?->nombre ?? 'N/A' }}</span>
                            @else
                        ...
                        <td class="px-5 py-3.5 text-center whitespace-nowrap">
                            <div class="inline-flex items-center justify-center space-x-1">
                                ...
                                <form action="{{ route('presentaciones.destroy', $pres) }}" method="POST" class="inline-flex items-center m-0 p-0" onsubmit="return confirm('¿Deseas eliminar esta presentación?');">
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="w-7 h-7 inline-flex items-center justify-center rounded-lg bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-400 transition"
