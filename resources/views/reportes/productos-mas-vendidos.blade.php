...
<div class="space-y-5" x-data="{ setDates(p){ const n=new Date(),f=new Date(),t=new Date(); if(p==='7d'){f.setDate(n.getDate()-6);}else if(p==='mes'){f.setDate(1);t.setMonth(n.getMonth()+1,0);}else if(p==='mesant'){f.setMonth(n.getMonth()-1,1);t.setMonth(n.getMonth(),0);}else if(p==='anio'){f.setMonth(0,1);t.setMonth(11,31);}const fmt=d=>d.toISOString().split('T')[0]; document.getElementById('pmv_d').value=fmt(f); document.getElementById('pmv_h').value=fmt(t); } }">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-300 dark:border-slate-800 shadow-xs print:hidden">
        <form method="GET" action="{{ route('reportes.productos-mas-vendidos') }}" class="flex flex-wrap items-end gap-3">
            <div>
                ...
                <input type="date" id="pmv_d" name="fecha_desde" value="{{ $fechaDesde }}" class="px-3 py-1.5 rounded-xl text-xs border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-400">
            </div>
            <div>
                <label for="pmv_h" class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Hasta</label>
                <input type="date" id="pmv_h" name="fecha_hasta" value="{{ $fechaHasta }}" class="px-3 py-1.5 rounded-xl text-xs border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-400">
            </div>
            @foreach([['Hoy','hoy'],['7 días','7d'],['Este mes','mes'],['Mes ant.','mesant'],['Año','anio']] as [$lb,$pr])
            <button type="button" @click="setDates('{{ $pr }}')"
                    class="px-2.5 py-1.5 rounded-xl text-xs font-bold border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:border-teal-400 hover:text-teal-700 dark:hover:text-teal-400 transition">{{ $lb }}</button>
            @endforeach
            <button type="submit" class="px-4 py-1.5 rounded-xl text-xs font-bold bg-teal-600 hover:bg-teal-700 text-white shadow-sm transition">Filtrar</button>
        </form>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-300 dark:border-slate-800 shadow-xs overflow-hidden">
        ...
        @if($ranking->isEmpty())
        <div class="flex flex-col items-center justify-center py-14"><p class="text-sm font-semibold text-slate-500 dark:text-slate-400">Sin ventas en este período</p></div>
        @else
            ...
            <table class="w-full text-xs">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                        ...
                        <td class="px-4 py-2.5 text-right"><span class="font-bold text-indigo-700 dark:text-indigo-400">{{ number_format($item->total_unidades_vendidas) }}</span> un.</td>
                        <td class="px-4 py-2.5 text-right font-bold text-emerald-700 dark:text-emerald-400">${{ number_format($item->total_ingresos, 2) }}</td>
                ...
                <tfoot>
                    <tr class="bg-emerald-50 dark:bg-emerald-950/20 border-t-2 border-emerald-200 dark:border-emerald-800">
                        <td colspan="3" class="px-4 py-2.5 text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Totales (Top {{ $ranking->count() }})</td>
                        <td class="px-4 py-2.5 text-right text-xs font-extrabold text-emerald-700 dark:text-emerald-400">{{ number_format($totalU) }} un.</td>
                        <td class="px-4 py-2.5 text-right text-xs font-extrabold text-emerald-700 dark:text-emerald-400">${{ number_format($totalI, 2) }}</td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
