...
<div x-data="{
    ...
    formData: (window.farmaGetDraft ? window.farmaGetDraft('{{ request()->getPathInfo() }}', {
        ...
        principio_activo: @js(old('principio_activo', '')),
        concentracion: @js(old('concentracion', '')),
        forma_farmaceutica: @js(old('forma_farmaceutica', '')),
        descripcion: @js(old('descripcion', '')),
        categoria_id: @js(old('categoria_id', '')),
        laboratorio_id: @js(old('laboratorio_id', '')),
        tipo_control: @js(old('tipo_control', 'receta_medica')),
        registro_sanitario: @js(old('registro_sanitario', '')),
        requiere_receta: @js(old('requiere_receta', false) ? true : false),
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
            ...
            <p class="text-xs text-slate-600 dark:text-slate-400">
                Ingresa la información farmacológica, presentaciones (blíster, caja, unidad), precios y fotografía.
            </p>
        </div>
        <a href="{{ route('productos.index') }}" 
           class="px-3.5 py-1.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold transition flex items-center space-x-1.5 shrink-0 self-start sm:self-auto shadow-2xs">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-5 items-center">
                            ...
                            <div class="sm:col-span-4 flex justify-center">
                                <div class="w-28 h-28 rounded-2xl border-2 border-dashed border-slate-400 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 flex items-center justify-center overflow-hidden shadow-2xs">
                                    ...
                                    <template x-if="!imgPreview">
                                        <span class="text-xs text-slate-400 text-center px-2">Vista previa</span>
                                    </template>
                                </div>
                            </div>
                        </div>
