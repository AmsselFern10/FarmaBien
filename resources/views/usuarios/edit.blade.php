@extends('layouts.app')
@section('title', 'Editar Usuario - FarmaBien')
@section('content')
<div class="max-w-2xl mx-auto space-y-5">
    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Editar Usuario</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ $usuario->name }} &middot; {{ $usuario->email }}</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('usuarios.show', $usuario) }}" class="px-3.5 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold rounded-xl transition">Ver Ficha</a>
    ...
    <form method="POST" action="{{ route('usuarios.update', $usuario) }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $usuario->name) }}" required
                            class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800/80 border @error('name') border-rose-400 @else border-slate-200 dark:border-slate-700 @enderror rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $usuario->email) }}" required
                            class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800/80 border @error('email') border-rose-400 @else border-slate-200 dark:border-slate-700 @enderror rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition">
                        @error('email')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">
                            Nueva contrasena <span class="text-slate-400 font-normal">(dejar vacio para no cambiar)</span>
                        </label>
                        <input type="password" name="password" autocomplete="new-password"
                            class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800/80 border @error('password') border-rose-400 @else border-slate-200 dark:border-slate-700 @enderror rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition"
                            placeholder="Nueva contrasena...">
                        @error('password')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Confirmar nueva contrasena</label>
                        <input type="password" name="password_confirmation" autocomplete="new-password"
                            class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" name="active" id="active" value="1" {{ old('active', $usuario->active) ? 'checked' : '' }}
        ...
        <div class="flex items-center justify-between gap-3">
            <a href="{{ route('usuarios.show', $usuario) }}" class="px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-xl transition">Cancelar</a>
