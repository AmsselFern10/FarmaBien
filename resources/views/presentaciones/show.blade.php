    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center space-x-2">
            ...
            <a href="{{ route('presentaciones.edit', $presentacion) }}"
               id="btn-editar-presentacion"
               aria-label="Editar presentación"
               class="inline-flex items-center space-x-1.5 px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl transition cursor-pointer">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                <span>Editar</span>
            </a>
            @endcan
            @can('eliminar productos')
            <form action="{{ route('presentaciones.destroy', $presentacion) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar esta presentación?');" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit" 
                        id="btn-eliminar-presentacion" 
                        aria-label="Eliminar presentación" 
                        class="inline-flex items-center space-x-1.5 px-3.5 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold rounded-xl transition cursor-pointer">
            ...
            <a href="{{ route('presentaciones.index') }}"
               id="btn-volver-presentaciones"
               aria-label="Volver a la lista de presentaciones"
