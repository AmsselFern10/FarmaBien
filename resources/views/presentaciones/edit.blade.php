...
@section('title', 'Editar Presentación - FarmaBien')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Editar: {{ $presentacion->nombre }}</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Medicamento: <span class="font-semibold text-emerald-600 dark:text-emerald-400">{{ $presentacion->producto->nombre }}</span>
            </p>
        </div>
        <div class="flex items-center space-x-2">
            <a href="{{ route('presentaciones.show', $presentacion) }}"
    ...
    <form method="POST" action="{{ route('presentaciones.update', $presentacion) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
            ...
            <div>
                ...
                <select id="producto_id" name="producto_id" required
                    ...
                    <option value="{{ $prod->id }}" {{ old('producto_id', $presentacion->producto_id) == $prod->id ? 'selected' : '' }}>{{ $prod->nombre }}</option>
                    @endforeach
                </select>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label for="nombre" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nombre <span class="text-rose-500">*</span></label>
                    <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $presentacion->nombre) }}" required
                ...
                <div>
                    <label for="unidades_por_presentacion" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Unidades por Presentación <span class="text-rose-500">*</span></label>
                    <input type="number" id="unidades_por_presentacion" name="unidades_por_presentacion" value="{{ old('unidades_por_presentacion', $presentacion->unidades_por_presentacion) }}" min="1" required
                ...
                <div>
                    ...
                    <input type="text" id="codigo_barras" name="codigo_barras" value="{{ old('codigo_barras', $presentacion->codigo_barras) }}" placeholder="EAN-13, UPC..."
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" id="precio_compra" name="precio_compra" step="0.01" min="0" value="{{ old('precio_compra', $presentacion->precio_compra) }}" placeholder="0.00"
                           class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition">
                </div>
                <div>
                    <label for="precio_venta" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Precio de Venta ($)</label>
                    <input type="number" id="precio_venta" name="precio_venta" step="0.01" min="0" value="{{ old('precio_venta', $presentacion->precio_venta) }}" placeholder="0.00"
                           class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
            ...
            <label class="flex items-center space-x-3 cursor-pointer">
                <input type="checkbox" name="es_unidad_base" value="1" {{ old('es_unidad_base', $presentacion->es_unidad_base) ? 'checked' : '' }}
            ...
            <label class="flex items-center space-x-3 cursor-pointer">
                <input type="checkbox" name="activo" value="1" {{ old('activo', $presentacion->activo) ? 'checked' : '' }}
        ...
        <div class="sticky bottom-0 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-3.5 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 shadow-lg z-20 flex items-center justify-between transition-all rounded-b-2xl">
            ...
            <button type="submit" class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-sm font-semibold rounded-xl shadow-sm transition inline-flex items-center space-x-2">
                ...
                <span>Actualizar Presentación</span>
            </button>
        </div>
    </form>
</div>
