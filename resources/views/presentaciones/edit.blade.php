...
function presFormEdit() {
    return {
        ...
        init() {
            ...
            this.nombre = "{{ old('nombre', $presentacion->nombre) }}";
            this.descripcion = "{{ old('descripcion', $presentacion->descripcion ?? '') }}";
            this.unidades = {{ old('unidades_por_presentacion', $presentacion->unidades_por_presentacion) }};
            this.precioCompra = "{{ old('precio_compra', $presentacion->precio_compra ?? '') }}";
            this.precioVenta = "{{ old('precio_venta', $presentacion->precio_venta ?? '') }}";
            this.codigoBarras = "{{ old('codigo_barras', $presentacion->codigo_barras ?? '') }}";
            this.esUnidadBase = {{ old('es_unidad_base', $presentacion->es_unidad_base) ? 'true' : 'false' }};
            this.activo = {{ old('activo', $presentacion->activo) ? 'true' : 'false' }};
            this.orden = {{ old('orden', $presentacion->orden) }};
            const oldId = "{{ old('producto_id', $presentacion->producto_id) }}";
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
    <div>
        <h1 class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">Editar Presentacion: {{ $presentacion->nombre }}</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400">Modifica los datos de la presentacion.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-800/30 space-y-3">
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <div>
                    <label for="pres_edit_compact_codigo_barras" class="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Cod. Barras</label>
                    <input type="text" id="pres_edit_compact_codigo_barras" name="codigo_barras" x-model="codigoBarras" placeholder="EAN-13..." class="w-full font-mono px-2.5 py-1.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white focus:ring-1 focus:ring-emerald-500">
                </div>
            </div>
            <div>
                <label for="pres_edit_compact_descripcion" class="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Descripcion</label>
                <textarea id="pres_edit_compact_descripcion" name="descripcion" x-model="descripcion" rows="2" placeholder="Descripcion adicional..." class="w-full px-2.5 py-1.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white focus:ring-1 focus:ring-emerald-500"></textarea>
            </div>
        </div>
        ...
        <div class="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-800/30 space-y-3">
            ...
            <div>
                <label for="pres_edit_compact_precio_compra" class="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Precio de Compra ($)</label>
                <input type="number" id="pres_edit_compact_precio_compra" name="precio_compra" x-model="precioCompra" step="0.01" min="0" placeholder="0.00" class="w-full font-mono px-2.5 py-1.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white focus:ring-1 focus:ring-emerald-500">
            </div>
            <div>
            ...
            <div>
                <label for="pres_edit_compact_orden" class="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Orden de Visualizacion</label>
                <input type="number" id="pres_edit_compact_orden" name="orden" x-model="orden" min="0" class="w-full px-2.5 py-1.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white focus:ring-1 focus:ring-emerald-500">
            </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="md:col-span-2">
                <label for="pres_edit_modern_descripcion" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Descripcion</label>
                <textarea id="pres_edit_modern_descripcion" name="descripcion" x-model="descripcion" rows="2" placeholder="Descripcion adicional..." class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition"></textarea>
            </div>
            <div>
                <label for="pres_edit_modern_codigo_barras" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Codigo de Barras</label>
                <input type="text" id="pres_edit_modern_codigo_barras" name="codigo_barras" x-model="codigoBarras" placeholder="EAN-13, UPC..." class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-mono text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition">
            </div>
            <div>
                <label for="pres_edit_modern_orden" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Orden de Visualizacion</label>
                <input type="number" id="pres_edit_modern_orden" name="orden" x-model="orden" min="0" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition">
            </div>
        </div>
    </div>
    {{-- Precios --}}
