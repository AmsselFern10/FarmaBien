        ...
        <form method="GET" action="{{ route('recetas.index') }}" class="flex flex-col gap-3">
            ...
            <div class="flex flex-col md:flex-row gap-3 items-center">
                ...
                <div class="flex items-center space-x-2 shrink-0">
                    <button type="submit" class="px-4 py-2 bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600 text-white text-xs font-semibold rounded-xl transition">Filtrar</button>
                    @if(request()->anyFilled(['buscar','estado','tipo_receta','fecha_desde','fecha_hasta']))
                    <a href="{{ route('recetas.index') }}" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-semibold rounded-xl transition">Limpiar</a>
                    @endif
                </div>
            </div>
        </form>
                        ...
                        <td class="px-5 py-3.5 text-right whitespace-nowrap">
                            <div class="inline-flex items-center justify-end space-x-1">
                                ...
                                <a href="{{ route('recetas.show', $rec) }}"
                                   class="inline-flex items-center justify-center w-7 h-7 text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800/80 rounded-lg transition" title="Ver">
                                ...
                                <a href="{{ route('recetas.edit', $rec) }}"
                                   class="inline-flex items-center justify-center w-7 h-7 text-slate-400 hover:text-emerald-400 hover:bg-slate-100 dark:hover:bg-slate-800/80 rounded-lg transition" title="Editar">
                                ...
                                <form method="POST" action="{{ route('recetas.destroy', $rec) }}" class="inline-flex m-0 p-0"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex items-center justify-center w-7 h-7 text-slate-400 hover:text-rose-400 hover:bg-slate-100 dark:hover:bg-slate-800/80 rounded-lg transition" title="Eliminar">
